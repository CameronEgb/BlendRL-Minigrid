...
class BlenderActor(nn.Module):
    ...
    def to_neural_action_distribution(self, neural_state):
        """
        Obtain action distribution from MiniGrid MLP neural policy.
        """

        # Forward pass through MiniGrid MLP
        logits = self.neural_actor(neural_state)  # -> (batch, 7)

        # BlendRL expects probabilities, so apply softmax
...
class BlenderActorCritic(nn.Module):
    ...
    def __init__(
        ...
        self.visual_neural_actor = module.MLP(
            device=device,
            out_size=self.env.n_raw_actions,   # should be 7
            logic=False
        )
        self.neural_critic = module.MLP(
            device=device,
            logic=False,
            out_size=1,
        )
    ...
    def get_neural_value(self, neural_state):
        """
        Compute the value using the neural value function from the MiniGrid MLP.
        """
        # neural_critic outputs (batch, 1)
        value = self.neural_critic(neural_state)
        return value
