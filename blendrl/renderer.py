...
class Renderer:
    ...
    def run(self):
        ...
        # MiniGrid NudgeEnv.reset returns (logic_state_batch, neural_state_batch)
        obs_logic, obs_nn = self.env.reset()
        obs_logic = th.as_tensor(obs_logic, device=self.model.device, dtype=th.float32)
        obs_nn = th.as_tensor(obs_nn, device=self.model.device, dtype=th.float32)
        ...
        while self.running:
            ...
            if not self.paused:
                ...
                if self.takeover:  # human plays game manually
                    ...
                    # BlendRL actor expects (neural_state, logic_state)
                    action, logprob = self.model.act(obs_nn, obs_logic)
                    value = self.model.get_value(obs_nn, obs_logic)

                # MiniGrid NudgeEnv.step returns:
                # ((logic_state, neural_state), rewards, truncations, dones, infos)
                (new_logic, new_nn), rewards, truncations, dones, infos = self.env.step(
                    action, is_mapped=self.takeover
                )

                # env.step returns unbatched (4,4) and (4,84,84); add batch dim back
                new_logic = th.as_tensor(
                    new_logic, device=self.model.device, dtype=th.float32
                ).unsqueeze(0)          # -> (1,4,4)

                new_nn = th.as_tensor(
                    new_nn, device=self.model.device, dtype=th.float32
                ).unsqueeze(0)          # -> (1,4,84,84)

                self._render()

                # rewards, dones are lists in your env
                reward = (
                    float(rewards[0])
                ...
                if self.takeover and reward != 0.0:
                    print(f"Reward {reward:.2f}")

                if self.reset:
                    done = True
                    obs_logic, obs_nn = self.env.reset()
                    obs_logic = th.as_tensor(
                        obs_logic, device=self.model.device, dtype=th.float32
                    )
                    obs_nn = th.as_tensor(
                        obs_nn, device=self.model.device, dtype=th.float32
                    )
                    self._render()

                obs_logic, obs_nn = new_logic, new_nn
                length += 1
                ret += reward
