...
class MLP(nn.Module):
    def __init__(self, device, hidden_sizes=[256, 256], has_softmax=False, has_sigmoid=False, out_size=2, as_dict=False, logic=False):
        ...
        # State features dimension for MIMIC
        self.num_in_features = 46 if not logic else 46
        ...
        for size in hidden_sizes:
            ...
            layers.append(nn.ReLU()) # Use ReLU for sepsis task
            last_size = size
    ...
    def _print(self):
        return "MIMIC Neural Agent (MLP) - State Dim: 46, Action Dim: 2"
